...
\begin{titlepage}
  \centering
  \vspace*{\fill}
  {\huge\bfseries PhD Problem Set 1, 2025\par}
  \vspace{1.5cm}
...
Additional data was downloaded from the FRED website, in particular the Federal Fund Target Rate (\texttt{DFEDTAR}). This target rate series was discontinued in December 2008, coinciding with the period of our analysis sample. The discontinued series was replaced by two new series, specifically the upper and lower limit of the federal funds target range (\texttt{DFEDTARU} and \texttt{DFEDTARL} respectively). To construct the $FF$ variable, the midpoint of the upper and lower limit series was calculated and collated with the target rate series ending in 2008. The rate data was then resampled to quarterly by taking the rate corresponding to the final day of the corresponding quarter.
% \vspace{10pt}
...
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccccccc@{}}
   ...
   \bottomrule
\end{tabular*}
\caption*{The table reports coefficients on $\Delta FF_t \times$ Bank HHI from regressions of log changes in balance sheet items (columns) on the interaction of the Fed funds shock and bank-level deposit market concentration. 
...
The row ``None'' uses the full sample without additional filtering; ``Growth'' drops bank–quarters in which total assets grow by more than 100\% quarter-on-quarter; ``Growth+Winsor'' additionally winsorizes the dependent variables at the 1st and 99th percentiles of their distributions. 
Standard errors (in parentheses) are clustered at the bank (rssdid) level.}
\label{tab:filtering}
\end{table}
...
Rows differ only in which fixed effects are included: the baseline row retains bank, quarter, and bank$\times$post-2008 fixed effects, while the alternative drops the bank$\times$post-2008 term but keeps bank and quarter fixed effects.}
\label{tab:x}
\end{table}
